...
class Customer(models.Model):
    ...
    def show_status(self):
        ...
        return mark_safe('<span style="background-color: {};color: white;padding: 2px">{}</span>'.format(status_color.get(self.status),self.get_status_display()))
...
class ClassList(models.Model):
    ...
    def show_Customer(self):
        return [str(i) for i in self.teachers.all()][0]
...
class ConsultRecord(models.Model):
    ...
    consultant = models.ForeignKey("UserProfile", verbose_name="销售", related_name='records')
